...
void run_length_encode(const char *input, char *output) {
    int count = 1;
    while (*input) {
        char current = *input;
...
void run_length_decode(const char *input, char *output) {
    while (*input) {
        char current = *input++;
        int count = 0;
...
void UPLOAD_command(int sockfd, const char *buffer)
{
    ...
    if (stat(file_path, &bufer) == 0) 
    {
        ...
        while ((bytes_read = fread(file_buffer, 1, sizeof(file_buffer), file)) > 0) {
            ...
            if (write(sockfd, file_buffer, bytes_read) < 0)
...
void DOWNLOAD_command(int sockfd, const char *buffer)
{
    ...
    while ((n = read(sockfd, file_reader, 100)) > 0) 
    {
        ...
        if (strncmp(file_reader, "eof", 3) == 0)
            break;
        printf("Writing\n");
        if (fwrite(file_reader, strlen(file_reader), 1, file) < 0)
